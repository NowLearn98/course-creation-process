    ...
    <Card className="group relative overflow-hidden border shadow-md hover:shadow-xl transition-all duration-300 bg-card">
      ...
      <CardContent className="relative">
        ...
        <div className="flex flex-wrap gap-3 mb-3">
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-gradient-to-br from-success/5 to-transparent border border-success/10">
            <DollarSign className="w-3.5 h-3.5 text-success" />
            <span className="text-[10px] text-muted-foreground">Revenue</span>
            <span className="text-sm font-bold text-foreground">${revenue.toLocaleString()}</span>
            <TrendingUp className="w-3 h-3 text-success" />
          </div>
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-gradient-to-br from-primary/5 to-transparent border border-primary/10">
            <Users className="w-3.5 h-3.5 text-primary" />
            <span className="text-[10px] text-muted-foreground">Students</span>
            <span className="text-sm font-bold text-foreground">{enrollments.toLocaleString()}</span>
          </div>
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-gradient-to-br from-accent/5 to-transparent border border-accent/10">
            <MousePointerClick className="w-3.5 h-3.5 text-accent" />
            <span className="text-[10px] text-muted-foreground">Clicks</span>
            <span className="text-sm font-bold text-foreground">{clicks.toLocaleString()}</span>
          </div>
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-gradient-to-br from-warning/5 to-transparent border border-warning/10">
            <Star className="w-3.5 h-3.5 text-warning fill-warning" />
            <span className="text-[10px] text-muted-foreground">Rating</span>
            <span className="text-sm font-bold text-foreground">{course.rating > 0 ? course.rating.toFixed(1) : 'N/A'}</span>
            {course.reviews > 0 && <span className="text-[10px] text-muted-foreground">({course.reviews})</span>}
          </div>
        </div>

        {/* Announcements & Attachments side by side */}
